function generateAdditionalMaterialsPACK() {
  ...
  if (response == ui.Button.YES) {
    ...
    // --- NEW: Equipment quantity check (F -> G) ---
    ...
    // --- NEW: Ductwork quantity check (K -> L) ---
    for (var k = 3; k <= 41; k++) {
    ...
    // Send email notification (PACK)
    var recipient = "wdigiacomo@packtimco.com";
    var subject   = "Additional Material Notification";
    var body      = "Additional material was added for the " + jobName + " job by " + person + ".\n\nLink to sheet: " + url;

    MailApp.sendEmail(recipient, subject, body);

    // Inform the user that the email has been sent
    ui.alert("Email sent to Wayne notifying additional material for the " + jobName + " job.");
  }
}
